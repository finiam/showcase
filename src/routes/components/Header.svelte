...
<header
  class="root fixed left-0 top-6 right-0 m-auto max-w-layout px-4 lg:px-7 flex justify-between items-center"
  class:header-hidden={headerHidden && y > 50}
>
  <a
    href="/"
    title="Finiam"
    class="block text-f-dark-gray z-10 transition-color"
  ...
  <nav
    class="nav fixed p-4 pt-24 lg:p-0 left-0 top-0 w-full lg:w-fit h-full bg-f-dark-gray lg:bg-transparent lg:static items-start lg:items-center gap-6 lg:gap-14 text-f-base flex flex-col lg:flex-row"
    class:menu-hidden={!mobileNavOpen}
    class:menu-visible={mobileNavOpen}
  >
    <a
  ...
  <button
    type="button"
    class="btn-toggle flex lg:hidden flex-col justify-between text-f-dark-gray z-10 transition-color"
    class:text-f-beige={mobileNavOpen}
